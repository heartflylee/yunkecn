$(function() {
	...
	$('#asselect').on('click',function() {
			...
			$('#edit-company i').removeClass('select-icon').addClass('select-icon-1');
		});
	$('#edit-company i').on('click',function() {
			var iconStyle = $(this).attr('class');
			if (iconStyle == 'select-icon') {
				$('#edit-company .selectBox').show();
				$('#edit-company i').removeClass('select-icon').addClass(
					'select-icon-1');
			} else if (iconStyle == 'select-icon-1') {
				$('#edit-company .selectBox').hide();
				$('#edit-company i').removeClass('select-icon-1').addClass(
					'select-icon');
			}
		});
	$('#edit-company .selectBox a').on('click', function() {
		...
		$('#edit-company i').removeClass('select-icon-1').addClass(
			'select-icon');
		$('#asselect').attr('value',
			$('#edit-company .selectBox a').eq(num).text());
	});
	// 模拟select1
	$('#asselect-1').on('click', function() {
		$('#c-box .selectBox').show();
		$('#c-box i').removeClass('select-icon').addClass('select-icon-1');
	});
	$('#c-box i').on('click', function() {
		...
		if (iconStyle == 'select-icon') {
			...
			$('#c-box i').removeClass('select-icon-1').addClass('select-icon');
		}
	});
	$('#c-box .selectBox a').on('click',function() {
			...
			$('#c-box .selectBox').hide();
			$('#c-box i').removeClass('select-icon-1').addClass('select-icon');
			$('#asselect-1').attr('value',$('#c-box .selectBox a').eq(num).text());
		});
	...
	$('#telphone-choose-box-1 #chooseBtn-1-1').on('click',function() {
				var dataLen = $('#telphone-choose-box-1 .select-box span i.choose-right').length;
				var arrs = [];
				for (var i = 0; i < dataLen; i++) {
					var arr = [];
					arr[0] = $(
							'#telphone-choose-box-1 .select-box span i.choose-right')
						.eq(i).siblings('b').text();
					arr[1] = 100 / dataLen;
					arrs[i] = arr;
				}
	...
	$('#telphone-choose-box-2 #chooseBtn-1-2').on('click',function() {
				var dataLen = $('#telphone-choose-box-2 .select-box span i.choose-right').length;
				var arrs = [];
				for (var i = 0; i < dataLen; i++) {
					var arr = [];
					arr[0] = $(
							'#telphone-choose-box-2 .select-box span i.choose-right')
						.eq(i).siblings('b').text();
					arr[1] = 100 / dataLen;
					arrs[i] = arr;
				}
	...
	$('#telphone-choose-box-3 #chooseBtn-1-3').on('click',function() {
				var dataLen = $('#telphone-choose-box-3 .select-box span i.choose-right').length;
				var arrs = [];
				for (var i = 0; i < dataLen; i++) {
					var arr = [];
					arr[0] = $(
							'#telphone-choose-box-3 .select-box span i.choose-right')
						.eq(i).siblings('b').text();
					arr[1] = 100 / dataLen;
					arrs[i] = arr;
				}
	...
	$('#telphone-choose-box-4 #chooseBtn-1-4').on('click',function() {
				var dataLen = $('#telphone-choose-box-4 .select-box span i.choose-right').length;
				var arrs = [];
				for (var i = 0; i < dataLen; i++) {
					var arr = [];
					arr[0] = $(
							'#telphone-choose-box-4 .select-box span i.choose-right')
						.eq(i).siblings('b').text();
					arr[1] = 100 / dataLen;
					arrs[i] = arr;
				}
	...
	$('#telphone-choose-box .select-box span').on('click',function() {
				var chooseStyle = $(this).find('i').attr('class');
				if (chooseStyle == "choose-null") {
					var len = $('#telphone-choose-box .select-box span i.choose-right').length;
					if (len < 8) {
						if (chooseStyle == 'choose-right') {
							$(this).find('i').removeClass(
								'choose-right').addClass(
								'choose-null');
					...
					if (chooseStyle == 'choose-right') {
						$(this).find('i').removeClass('choose-right')
							.addClass('choose-null');
					} else if (chooseStyle == 'choose-null') {
						$(this).find('i').removeClass('choose-null')
							.addClass('choose-right');
					}
				}
			});
	$('#telphone-choose-box-1 .select-box span').on('click',function() {
				var chooseStyle = $(this).find('i').attr('class');
				var len = $('#telphone-choose-box-1 .select-box span i.choose-right').length;
				if (len < 8) {
					if (chooseStyle == 'choose-right') {
						$(this).find('i').removeClass('choose-right')
							.addClass('choose-null');
	...
	$('#telphone-choose-box-2 .select-box span').on('click',function() {
				var chooseStyle = $(this).find('i').attr('class');
				var len = $('#telphone-choose-box-2 .select-box span i.choose-right').length;
				if (len < 8) {
					if (chooseStyle == 'choose-right') {
						$(this).find('i').removeClass('choose-right')
							.addClass('choose-null');
	...
	$('#telphone-choose-box-3 .select-box span').on('click',function() {
				var chooseStyle = $(this).find('i').attr('class');
				var len = $('#telphone-choose-box-3 .select-box span i.choose-right').length;
				if (len < 8) {
					if (chooseStyle == 'choose-right') {
						$(this).find('i').removeClass('choose-right')
							.addClass('choose-null');
	...
	$('#telphone-choose-box-4 .select-box span').on('click',function() {
				var chooseStyle = $(this).find('i').attr('class');
				var len = $('#telphone-choose-box-4 .select-box span i.choose-right').length;
				if (len < 8) {
					if (chooseStyle == 'choose-right') {
						$(this).find('i').removeClass('choose-right')
							.addClass('choose-null');
	...
	$('.table-list tr:odd').hover(function() {
		$(this).css({
			'background': '#f7fdfc'
		});
	}, function() {
	...
	$('.table-list tr:even').hover(function() {
		$(this).css({
			'background': '#f7fdfc'
		});
	}, function() {
	...
	$('#table-list-1 th i').on('click', function() {
		$(this).toggleClass('choose-all');
		if ($(this).hasClass("choosen")) {
			$('#table-list-1 th i').addClass('choose-all').removeClass('choosen');
	...
	$('#table-list-1 td i').on('click', function() {
		...
		if (len > lenCur && lenCur != 0) {
			$('#table-list-1 th i').removeClass('choose-all').addClass('choosen');
		}
	});
	...
	$('#table-list-2 th i').on('click', function() {
		$(this).toggleClass('choose-all');
		if ($(this).hasClass("choosen")) {
			$('#table-list-2 th i').addClass('choose-all').removeClass('choosen');
	...
	$('#table-list-2 td i').on('click', function() {
		...
		if (len > lenCur && lenCur != 0) {
			$('#table-list-2 th i').removeClass('choose-all').addClass('choosen');
		}
	});
	...
	$('#table-list-3 th i').on('click', function() {
		$(this).toggleClass('choose-all');
		if ($(this).hasClass("choosen")) {
			$('#table-list-3 th i').addClass('choose-all').removeClass('choosen');
	...
	$('#table-list-3 td i').on('click', function() {
		...
		if (len > lenCur && lenCur != 0) {
			$('#table-list-3 th i').removeClass('choose-all').addClass('choosen');
		}
	});
	...
	$('#changer-box .a-closed,#changer-box #changerBtn').on('click',
		function() {
			$('#changer-box').hide();
			$('.layer').hide();
	...
	$('#changer-box-2 .a-closed,#changer-box-2 #changerBtn-2').on('click',
		function() {
			$('#changer-box-2').hide();
			$('.layer').hide();
	...
	$('#changer-box-1 .a-closed,#changer-box-1 #changerBtn-1,#changer-box-1 #changerBtn-1-1').on('click', function() {
			$('#changer-box-1').hide();
			$('.layer').hide();
	...
	$('#customHeaderBox #customBtn,#customHeaderBox .a-closed').on('click',
		function() {
			$('#customHeaderBox').hide();
			$('.layer').hide();
		});
	$('#hiddenField ul li,#showField ul li').toggle(function() {
...
//	});
	...
	$('#tel-edit').on('click',function() {
		$('.telLayerBox .contextBox input,.telLayerBox .contextBox textarea').attr("readonly", false);
		$('.telLayerBox .contextBox input,.telLayerBox .contextBox textarea').css({'border-width': '1px','background': '#fff'});
		$(".address_input").hide();
		$(".address_sel").show();
//		$('#hangupBtn,#p-time').hide();
	...
	$('#saveBtn_call').on('click',function() {
		$('.telLayerBox .contextBox input,.telLayerBox .contextBox textarea').attr("readonly", true);
		$('.telLayerBox .contextBox input,.telLayerBox .contextBox textarea').css({'border-width': '0px','background': 'none'});
		$(".address_input").show();
		$(".address_sel").hide();
//		$('#hangupBtn,#p-time').show();
	 ...
	 */
	// 任务滑动样式
	$('.overdue-status').hover(function() {
		$(this).find('i').addClass('redcolor');
	}, function() {
		$(this).find('i').removeClass('redcolor');
	});
	$('.overdue-status-1').hover(function() {
		$(this).find('i').addClass('redcolor');
	}, function() {
		$(this).find('i').removeClass('redcolor');
	});
	$('.progress-status').hover(function() {
		$(this).find('i').addClass('bluecolor');
	}, function() {
		$(this).find('i').removeClass('bluecolor');
	});
	$('.others-status').hover(function() {
		$(this).find('i').addClass('graycolor');
	}, function() {
		$(this).find('i').removeClass('graycolor');
	});
	...
	$('#c-pay-1 .c-a').on('click', function() {
		$('#c-pay-1 .c-a').removeClass('buy-items-cur').addClass('buy-items');
	 ...
	 */
	// 外呼标签设置
	$('.dl-chosen dd span').hover(function() {
...
//	});
	...
	$(".choose-members .del-add").click(function(event) {
		var left_btn = $(this).offset().left + $(this).outerWidth() + 10 - $(".choose-members").offset().left;
		$(".add_employees").css("left", left_btn).show();
		$(document).one("click", function() {
			$(".add_employees").hide();
		});
		event.stopPropagation();
...
//设置中标签位置
	$(".dl-chosen .add").click(function(){
		...
		var chosen_top=0;
		var chosen_left=0;
		if($(this).offset().top+$("#addtip").outerHeight()>$("body").height()){
			chosen_top=$(this).offset().top+$(this).outerHeight()-$("#addtip").outerHeight();
		}else{
			chosen_top=$(this).offset().top;
		}
		if($(this).offset().left+$(this).outerWidth()+$("#addtip").outerWidth()>$("body").width()){
			chosen_left=$(this).offset().left-$("#addtip").outerWidth();
		}else{
			chosen_left=$(this).offset().left+$(this).outerWidth();
		}
		$("#addtip").css({"top":chosen_top,"left":chosen_left}).show();
	});
	...
	$(".edit-small").click(function(){
		...
		var chosen_top=0;
		var chosen_left=0;
		if($(this).parent().offset().top+$("#updatetip").outerHeight()>$("body").height()){
			chosen_top=$(this).parent().offset().top+$(this).parent().outerHeight()-$("#updatetip").outerHeight();
		}else{
			chosen_top=$(this).parent().offset().top;
		}
		if($(this).parent().offset().left+$(this).parent().outerWidth()+$("#updatetip").outerWidth()>$("body").width()){
			chosen_left=$(this).parent().offset().left-$("#updatetip").outerWidth();
		}else{
			chosen_left=$(this).parent().offset().left+$(this).parent().outerWidth();
		}
		
		$("#updatetip").css({"top":chosen_top,"left":chosen_left}).show();
	});
	$(".delete-small").click(function(){
		$(".taglayer").hide();
		var chosen_top=0;
		var chosen_left=0;
		if($(this).parent().offset().top+$("#deletetip").outerHeight()>$("body").height()){
			chosen_top=$(this).parent().offset().top+$(this).parent().outerHeight()-$("#deletetip").outerHeight();
		}else{
			chosen_top=$(this).parent().offset().top;
		}
		if($(this).parent().offset().left+$(this).parent().outerWidth()+$("#deletetip").outerWidth()>$("body").width()){
			chosen_left=$(this).parent().offset().left-$("#deletetip").outerWidth();
		}else{
			chosen_left=$(this).parent().offset().left+$(this).parent().outerWidth();
		}
		$("#deletetip").css({"top":chosen_top,"left":chosen_left}).show();
	});



$(".layer,.layer-1,.layer-2").click(function(){
	if($(".telLayerBox-1").css("display")=="block"||$(".telLayerBox-1").css("display")=="block"||$(".telLayerBox-2").css("display")=="block"||$(".telLayerBox").css("display")=="block"){
		return;
	}
	$(".customHeaderBox,.changer-box,.applyBox,.introLayerBoxlys,.introLayerBox,.telphone-choose-box").hide();
	$('#filter').click();
	$(".layer,.layer-1,.layer-2").hide();
})
