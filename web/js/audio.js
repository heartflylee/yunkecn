	...
	g[o].prototype = {
		...
		loadProgress: function() {
			...
			var c = setInterval(function() {
				...
				//					console.log(this.element.buffered);
				//					console.log(this.element.buffered.end(this.element.buffered.length - 1));
				//
				//					//				this.loadedPercent = this.element.buffered.end(0) / this.duration;
				//					this.loadedPercent = this.element.buffered.end(this.element.buffered.length - 1) / this.duration;
				//					this.settings.loadProgress.apply(this, [this.loadedPercent]);
				//					console.log();
				//				}
				if(obj.element.buffered != null && obj.element.buffered.length) {
					if(!obj.loadStartedCalled) obj.loadStartedCalled = obj.loadStarted();
					obj.loadedPercent = obj.element.buffered.end(obj.element.buffered.length - 1) / obj.duration;
					obj.settings.loadProgress.apply(obj, [obj.loadedPercent]);
				}
				if(obj.loadedPercent >= 1) {
					clearInterval(c);
				}
			}, 10);
